...
**Never stamp `depth` from more than one thread.** HDF5 is commonly built
...
execution. This is why `DepthStamper` returns `parallel() -> false`.
...
- The `netcdf` crate already takes an exclusive process-wide lock around every
  ...
  `DepthStamper` is belt and braces, kept because `silence_hdf5_diagnostics`
